...
static void* event_loop_thread(void* arg)
{
    ...
    if (!g_effective_cores.empty()) {
        int core_idx = g_effective_cores[event_loop_idx % g_effective_cores.size()];
        set_thread_affinity(core_idx);
    }
...
void init_thread_event_loops(int io_thread_num, bool set_cpu_affinity)
{
    ...
    if (set_cpu_affinity) {
        g_effective_cores = get_effective_cores();
    }
